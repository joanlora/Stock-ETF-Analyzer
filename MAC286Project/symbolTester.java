...
public class symbolTester {
	...
	void outcomes(Trade T, int ind) {
		...
		for (int i = ind; i < mData.size(); i++) {
			if (T.getDir() == Direction.LONG) {
				...
				T.close(mData.elementAt(mData.size() - 1).getDate(), mData.elementAt(mData.size() - 1).getClose(),
						mData.size() - 1 - ind);
	...
	public boolean test() {
		...
		/*
		 * As an example let's test the following pattern 1- today makes a 10 days low
		 * 2- today is a outside bar (reversal) today's low is smaller than yesterday's
		 * low and today's high is larger than yesterday's high. 3- today's close near
		 * the high (within less than 10%) (high-close)/(high-low)<0.1; 4- buy at open
		 * tomorrow and stop today's low and target factor*risk
		 */

		for (int i = 10; i < mData.size() - 1; i++) {
			if (xDaysLow(i, 10) && mData.elementAt(i).getLow() < mData.elementAt(i - 1).getLow()
					&& mData.elementAt(i).getHigh() > mData.elementAt(i - 1).getHigh()
					&& (mData.elementAt(i).getHigh() - mData.elementAt(i).getClose())
							/ (mData.elementAt(i).getHigh() - mData.elementAt(i).getLow()) < 0.1) {
				// we have a trade, buy at opne of i+1 (tomorrow) stoploss i.low, target =
				// entry+factor*risk
				...
				mTrades.add(T);
			} else if ((xDaysHigh(i, 10) && mData.elementAt(i).getHigh() > mData.elementAt(i - 1).getHigh()
					&& mData.elementAt(i).getLow() < mData.elementAt(i - 1).getLow()
					&& (mData.elementAt(i).getClose() - mData.elementAt(i).getLow())
							/ (mData.elementAt(i).getHigh() - mData.elementAt(i).getLow()) < 0.1)) {
